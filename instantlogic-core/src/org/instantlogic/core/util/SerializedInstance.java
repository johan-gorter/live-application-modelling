...
public class SerializedInstance<I extends Instance<I>> implements Serializable {
	...
	public static <II extends Instance<II>> SerializedInstance<II> serialize(II src) {
		Entity<II> entity = src.getModel();
		long id = src.getInstanceId();
	...
	public void deserialize(I target) {
		if (entityName.equals(target.getModel().getName())) {
			throw new RuntimeException("Wrong entity name. was: "+getEntityName()+" expected: "+target.getModel().getName());
		}
	...
	private static <II extends Instance<II>> void deserializeFirstPass(SerializedInstance<II> source, II target) {
		Entity<II> entity = target.getModel();
		for (Attribute<II, ?, ?> attribute : entity.getAttributes()) {
			if (!attribute.isReadOnly()) {
	...
	private static <II extends Instance<II>> void deserializeSecondPass(SerializedInstance<II> source, II target) {
		Entity<II> entity = target.getModel();
		InstanceRegistry registry = target.getCase().getRegistry();
		for (Relation relation : entity.getRelations()) {
			if (!relation.isReadOnly()) {
				if (source.values.containsKey(relation.getName())) {
					...
					if (!relation.isOwner()) {
						...
						if (relation.isMultivalue()) {
							...
							for (Object item: ((Object[])value)) {
								values.add(registry.getInstanceById((Long)item));
							}
						} else {
							relationValue.set(registry.getInstanceById((Long)value));
						}
					} else {
						// Recurse
						if (relation.isMultivalue()) {
	...
	private static Instance<?> createInstance(org.instantlogic.core.CaseInstance<?> caseInstance, String entityName, long id) {
		Entity<?> entity = caseInstance.getModel().getEntities().get(entityName);
		return entity.createInstance(caseInstance, id);
	}
